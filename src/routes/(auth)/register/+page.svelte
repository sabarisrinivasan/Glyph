...
<main class="flex h-[80dvh] items-center justify-center">
	...
	>
		...
		<div>
			...
			<input type="email" class={`input w-full ${emailError? "input-error" :""}`} value={data?.form?.data?.email ?? ""} placeholder="email" name="email" id="email" />
		...
		<button class="btn w-full btn-primary">register</button>
	</form>
</main>
